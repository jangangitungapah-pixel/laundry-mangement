...
`src/styles/tokens.css` adalah sumber token. Komponen harus memakai semantic custom properties atau pemetaan Tailwind, bukan menanam warna mentah.

### Warna semantik

| Token            | Nilai baseline | Pemakaian                |
| ---------------- | -------------- | ------------------------ |
| `background`     | `#f4f8f7`      | Latar aplikasi           |
| `surface`        | `#ffffff`      | Panel dan dialog         |
| `surface-subtle` | `#edf4f2`      | Kelompok konten sekunder |
| `text`           | `#132522`      | Teks utama               |
| `muted`          | `#5c6e69`      | Teks penjelas            |
| `border`         | `#cbd9d5`      | Pemisah dan kontrol      |
| `primary`        | `#0f766e`      | Aksi utama               |
| `accent`         | `#ccfbf1`      | Penanda pilihan          |
| `success`        | `#18794e`      | Hasil berhasil           |
| `warning`        | `#9a5807`      | Perhatian                |
| `danger`         | `#b4232e`      | Risiko/destructive       |
| `info`           | `#1e5f91`      | Informasi netral         |

Kontras teks normal terhadap surface/background wajib memenuhi WCAG AA. Perubahan token warna memerlukan pengukuran ulang seluruh pasangan foreground/surface.
...
Perubahan visual juga memerlukan QA representatif pada 1440 × 900, 768 × 1024, dan 360 × 800, keyboard/focus smoke test, serta print QA 58/80 mm. Bukti baseline terdapat pada [Design System Audit](DESIGN_SYSTEM_AUDIT.md).
